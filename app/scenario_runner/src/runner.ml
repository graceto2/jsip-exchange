  ...
  print_endline
    [%string
      "[scenario] starting %{config.name} on port %{port#Int} \
       (seed=%{seed#Int})"];
  (* Scenarios name their symbols by id ([config.symbols]), so placeholder
     tickers suffice — nothing in a scenario renders one. *)
  let directory =
    Symbol_directory.numbered ~num_symbols:(List.length config.symbols)
  in
