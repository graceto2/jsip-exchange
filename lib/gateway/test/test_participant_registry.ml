(** Unit tests for {!Participant_registry}: interning participant names to
    ...
    The registry is additive and idempotent — a name keeps its id for the
    whole run and ids are handed out densely from 0 — so these tests pin both
    the id assignment and the name round-trip. *)
...
let%expect_test "name_of_id resolves an id back to the name it was interned \
  ...
  print_s
    [%sexp (Participant_registry.name_of_id t alice_id : Participant.t)];
  [%expect {| Alice |}];
