...
(* Implementation is a plain [int]; the [.mli] narrows it to [private int] so
   that only this module (and, later, the login registry built on it) can
   mint ids, while the rest of the server may still read one as an int. *)
...
module Generator = struct
  ...
  (* Starts at 0 so an id doubles as its index in the registry's id->name
     [Queue] — the k-th interned participant gets id [k] and sits at slot
     [k]. (Unlike [Order_id.Generator], which starts at 1.) *)
  let create () = { next = 0 }
