...
let%expect_test "partial fill: buy is larger than resting sell" =
  ...
  [%expect
    ...
    ACCEPTED id=2 0 BUY 100@$150.00 DAY
    FILL fill_id=1 aggressor_client_oid=1 resting_client_oid=0 0 $150.00 x60 aggressor=2(Alice) BUY resting=1(Bob)
    |}];
...
let%expect_test "aggressor sweeps multiple resting orders" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=2 resting_client_oid=0 0 $150.00 x50 aggressor=3(Alice) BUY resting=1(Bob)
    FILL fill_id=2 aggressor_client_oid=2 resting_client_oid=1 0 $150.00 x50 aggressor=3(Alice) BUY resting=2(Charlie)
    |}]
;;
...
let%expect_test "IOC: no match means immediate cancel" =
  ...
  [%expect
    ...
    CANCELLED order_id=1 client_oid=0 0 remaining=100 reason=IOC_REMAINDER
    |}]
;;
...
let%expect_test "IOC: partial fill then cancel remainder" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=1 resting_client_oid=0 0 $150.00 x40 aggressor=2(Alice) BUY resting=1(Bob)
    CANCELLED order_id=2 client_oid=1 0 remaining=60 reason=IOC_REMAINDER
    |}]
;;
...
let%expect_test "price priority: naive impl matches first-found, not best" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=2 resting_client_oid=0 0 $10.00 x100 aggressor=3(Alice) BUY resting=1(Charlie)
    |}]
;;
...
let%expect_test "scenario: two participants trade, book reflects state" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=4 resting_client_oid=2 0 $150.10 x50 aggressor=5(Charlie) BUY resting=3(Bob)
    === 0 ===
      BIDS:
        $149.90 x100
        $149.80 x200
...
let%expect_test "scenario: aggressive IOC sweeps entire book" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=3 resting_client_oid=0 0 $150.00 x50 aggressor=4(Alice) BUY resting=1(Bob)
    FILL fill_id=2 aggressor_client_oid=3 resting_client_oid=1 0 $150.10 x50 aggressor=4(Alice) BUY resting=2(Charlie)
    FILL fill_id=3 aggressor_client_oid=3 resting_client_oid=2 0 $150.20 x50 aggressor=4(Alice) BUY resting=3(Bob)
    CANCELLED order_id=4 client_oid=3 0 remaining=50 reason=IOC_REMAINDER
    === 0 ===
      BIDS: (empty)
      ASKS: (empty)
      BBO: - / -
...
let%expect_test "scenario: fill IDs are globally sequential" =
  ...
  [%expect
    ...
    ACCEPTED id=2 1 SELL 100@$200.00 DAY
    ACCEPTED id=3 0 BUY 100@$150.00 DAY
    FILL fill_id=1 aggressor_client_oid=2 resting_client_oid=0 0 $150.00 x100 aggressor=3(Alice) BUY resting=1(Bob)
    ACCEPTED id=4 1 BUY 100@$200.00 DAY
    FILL fill_id=2 aggressor_client_oid=3 resting_client_oid=1 1 $200.00 x100 aggressor=4(Alice) BUY resting=2(Charlie)
    |}]
;;
...
let%expect_test "duplicate detection: alice can't make duplicate client \
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 100@$150.00 DAY
    REJECTED 0 SELL 100@$10.00 reason=Client order ID already in use
    |}]
;;
