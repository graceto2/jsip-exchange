...
let%expect_test "partial fill: buy is larger than resting sell" =
  ...
  [%expect
    ...
    ACCEPTED id=2 AAPL BUY 100@$150.00 DAY
    FILL fill_id=1 aggressor_client_oid=1 resting_client_oid=0 AAPL $150.00 x60 aggressor=2(Alice) BUY resting=1(Bob)
    |}];
...
let%expect_test "aggressor sweeps multiple resting orders" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=2 resting_client_oid=0 AAPL $150.00 x50 aggressor=3(Alice) BUY resting=1(Bob)
    FILL fill_id=2 aggressor_client_oid=2 resting_client_oid=1 AAPL $150.00 x50 aggressor=3(Alice) BUY resting=2(Charlie)
    |}]
;;
...
let%expect_test "IOC: no match means immediate cancel" =
  ...
  [%expect
    ...
    CANCELLED order_id=1 client_oid=0 AAPL remaining=100 reason=IOC_REMAINDER
    |}]
;;
...
let%expect_test "IOC: partial fill then cancel remainder" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=1 resting_client_oid=0 AAPL $150.00 x40 aggressor=2(Alice) BUY resting=1(Bob)
    CANCELLED order_id=2 client_oid=1 AAPL remaining=60 reason=IOC_REMAINDER
    |}]
;;
...
let%expect_test "price priority: naive impl matches first-found, not best" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=2 resting_client_oid=0 AAPL $10.00 x100 aggressor=3(Alice) BUY resting=1(Charlie)
    |}]
;;
...
let%expect_test "scenario: two participants trade, book reflects state" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=4 resting_client_oid=2 AAPL $150.10 x50 aggressor=5(Charlie) BUY resting=3(Bob)
    === AAPL ===
      BIDS:
        $149.90 x100
        $149.80 x200
...
let%expect_test "scenario: aggressive IOC sweeps entire book" =
  ...
  [%expect
    ...
    FILL fill_id=1 aggressor_client_oid=3 resting_client_oid=0 AAPL $150.00 x50 aggressor=4(Alice) BUY resting=1(Bob)
    FILL fill_id=2 aggressor_client_oid=3 resting_client_oid=1 AAPL $150.10 x50 aggressor=4(Alice) BUY resting=2(Charlie)
    FILL fill_id=3 aggressor_client_oid=3 resting_client_oid=2 AAPL $150.20 x50 aggressor=4(Alice) BUY resting=3(Bob)
    CANCELLED order_id=4 client_oid=3 AAPL remaining=50 reason=IOC_REMAINDER
    === AAPL ===
      BIDS: (empty)
      ASKS: (empty)
      BBO: - / -
...
let%expect_test "scenario: fill IDs are globally sequential" =
  ...
  [%expect
    ...
    ACCEPTED id=2 TSLA SELL 100@$200.00 DAY
    ACCEPTED id=3 AAPL BUY 100@$150.00 DAY
    FILL fill_id=1 aggressor_client_oid=2 resting_client_oid=0 AAPL $150.00 x100 aggressor=3(Alice) BUY resting=1(Bob)
    ACCEPTED id=4 TSLA BUY 100@$200.00 DAY
    FILL fill_id=2 aggressor_client_oid=3 resting_client_oid=1 TSLA $200.00 x100 aggressor=4(Alice) BUY resting=2(Charlie)
    |}]
;;
...
let%expect_test "duplicate detection: alice can't make duplicate client \
                 order ids"
  =
  let t = Harness.create () in
  submit_
...
let%expect_test "end_of_day cancels resting orders and empties the books" =
  let t = Harness.create () in
  Harness.submit_quiet_
    t
    (Harness.buy ~price_cents:14990 ~participant:Harness.alice ());
  Harness.submit_quiet_
    t
    (Harness.sell ~price_cents:15010 ~participant:Harness.bob ());
  Harness.submit_quiet_
    t
    (Harness.buy
       ~price_cents:24990
       ...
       ~participant:Harness.alice
       ());
  print_endline "--- before close ---";
  Harness.print_book t Harness.aapl;
  Harness.print_events (Matching_engine.end_of_day (Harness.engine t));
  ...
  [%expect
    ...
    CANCELLED order_id=1 client_oid=0 AAPL remaining=100 reason=END_OF_DAY
    CANCELLED order_id=2 client_oid=1 AAPL remaining=100 reason=END_OF_DAY
    BBO AAPL bid=- ask=-
    CANCELLED order_id=3 client_oid=2 TSLA remaining=100 reason=END_OF_DAY
    BBO TSLA bid=- ask=-
    --- after close ---
    === AAPL ===
      BIDS: (empty)
...
(* The bug this guards against: emitting the cancellations but leaving the
   orders in the book, so the exchange keeps matching against orders it has
   already announced as dead. A buy that would have crossed Bob's resting
   sell must now rest untouched. *)
let%expect_test "end_of_day: a cancelled order cannot be matched afterwards" =
  let t = Harness.create () in
  Harness.submit_quiet_
    t
    (Harness.sell ~price_cents:15000 ~participant:Harness.bob ());
  ignore
    (Matching_engine.end_of_day (Harness.engine t) : Exchange_event.t list);
  submit_ t (Harness.buy ~price_cents:15000 ~participant:Harness.alice ());
  [%expect {| ACCEPTED id=2 AAPL BUY 100@$150.00 DAY |}]
;;
